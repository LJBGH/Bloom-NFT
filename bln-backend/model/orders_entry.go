...
type EntryOrders struct {
	...
	MerkleRoot      string              `json:"merkleRoot" gorm:"type:varchar(66);not null;default:''"`         // 根哈希
	RootDeadlineSec uint64              `json:"rootDeadlineSec" gorm:"type:bigint unsigned;not null;default:0"` // 根截止时间
	MerkleProof     string              `json:"merkleProof" gorm:"type:longtext;null"`                          // Merkle proof
	ListingHash     string              `json:"listingHash" gorm:"type:varchar(66);not null;uniqueIndex"`       // 链上 listingHash
